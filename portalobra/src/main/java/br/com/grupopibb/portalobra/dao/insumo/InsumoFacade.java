...
public class InsumoFacade extends AbstractEntityBeans<Insumo, Long> {
    ...
    public List<Insumo> findRangeParam(final String codigo, final String especificacao, final String caracterizacao, final String classe, final String grupo, final boolean obraLinkadaOrcamento, final boolean desconsideraLinkOrcamento, final Integer planCod, final int[] range) {
        Map<String, Object> params = getMapParams();
        paramsFiltros(params, codigo, especificacao, caracterizacao, classe, grupo);
        if (obraLinkadaOrcamento && !desconsideraLinkOrcamento) {
            paramsOrc(params, planCod);
        }
        return listPesqParamRange(obraLinkadaOrcamento && !desconsideraLinkOrcamento ? "Insumo.findParamOrc" : "Insumo.findParam", params, range[1] - range[0], range[0]);
    }
    ...
    private void paramsOrc(Map<String, Object> params, final int projCod) {
        params.put("planCod", projCod);
    }
